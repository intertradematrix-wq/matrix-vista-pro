...
showcase = showcase.replace(/to=\{\`\/industry\/\$\{ind\.slug\}\`\}/g, 'to="/industry/$slug" params={{ slug: ind.slug }}');
...
productsTs = productsTs.replace(/label: "Description Text"/g, 'label: "ข้อความอธิบายแบบย่อ (Short Text)"');
// rename label: "Description HTML" to "Full Description (Rich Text Editor)"
productsTs = productsTs.replace(/label: "Description HTML"/g, 'label: "รายละเอียดแบบเต็ม (Rich Text Editor)"');
